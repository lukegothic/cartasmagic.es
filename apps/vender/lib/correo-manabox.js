...
const cuerpoHtml = ({ lead, mazo, presupuesto }) => [
  ...
  parrafo(MANABOX.porQueBaja.intro),
  lista(MANABOX.porQueBaja.enlaces.map(({ texto, url }) => `<a href="${url}" style="color:#f5901e;">${texto}</a>`)),
...
const cuerpoTexto = ({ lead, mazo, presupuesto }) => [
  ...
  '',
  MANABOX.porQueBaja.intro,
  ...MANABOX.porQueBaja.enlaces.map(({ texto, url }) => `  - ${texto}: ${url}`),
  '',
